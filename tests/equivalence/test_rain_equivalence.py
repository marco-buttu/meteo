"""Equivalence tests for legacy_rain and the native Python rain operation."""
...
LEGACY_SIX_DECIMAL_FIELDS = {"rain_mm"}
...
def _rain_config(config: dict[str, Any]) -> dict[str, Any]:
    return dict(config.get("rain", {}))


def _valid_cases(config: dict[str, Any]) -> list[dict[str, Any]]:
    return list(_rain_config(config).get("valid_cases", []))


def _configured_dates(config: dict[str, Any]) -> list[str]:
    return list(_rain_config(config).get("all_valid_epochs_for_dates", []))


def _async_failure_cases(config: dict[str, Any]) -> list[dict[str, Any]]:
    return list(_rain_config(config).get("async_failure_cases", []))


def _request_validation_cases(config: dict[str, Any]) -> list[dict[str, Any]]:
    return list(_rain_config(config).get("request_validation_cases", []))


def _run_pair(client: ApiClient, parameters: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    legacy = client.run_operation("legacy_rain", parameters)
    native = client.run_operation("rain", parameters)
...
def _series_length(client: ApiClient, date: str) -> int:
    result = client.run_operation("legacy_rain", {"date": date, "hour": 0})
...
def test_rain_configured_valid_cases_match_legacy(equivalence_config: dict[str, Any]) -> None:
...
def test_rain_all_valid_epochs_match_legacy_for_configured_dates(equivalence_config: dict[str, Any]) -> None:
    client = _client(equivalence_config)
    abs_tol = float(equivalence_config.get("abs_tol", 1e-6))
    rel_tol = float(equivalence_config.get("rel_tol", 1e-6))
...
def test_rain_expected_async_failures_match_legacy(equivalence_config: dict[str, Any]) -> None:
    client = _client(equivalence_config)
...
@pytest.mark.parametrize("operation", ["legacy_rain", "rain"])
def test_rain_request_validation_cases(equivalence_config: dict[str, Any], operation: str) -> None:
    client = _client(equivalence_config)
