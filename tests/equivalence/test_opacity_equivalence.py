"""Equivalence tests for legacy_opacity and the native Python opacity operation."""
...
LEGACY_SIX_DECIMAL_FIELDS = {"tau_np", "tmean_k"}
LEGACY_MJD_DECIMAL_FIELDS = {"mjd"}
LEGACY_THREE_DECIMAL_FIELDS = {"freq_ghz"}
...
def _opacity_config(config: dict[str, Any]) -> dict[str, Any]:
    return dict(config.get("opacity", {}))


def _valid_cases(config: dict[str, Any]) -> list[dict[str, Any]]:
    return list(_opacity_config(config).get("valid_cases", []))


def _configured_sweeps(config: dict[str, Any]) -> list[dict[str, Any]]:
    return list(_opacity_config(config).get("all_valid_epochs_for_cases", []))


def _async_failure_cases(config: dict[str, Any]) -> list[dict[str, Any]]:
    return list(_opacity_config(config).get("async_failure_cases", []))


def _request_validation_cases(config: dict[str, Any]) -> list[dict[str, Any]]:
    return list(_opacity_config(config).get("request_validation_cases", []))


def _run_pair(client: ApiClient, parameters: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    legacy = client.run_operation("legacy_opacity", parameters)
    native = client.run_operation("opacity", parameters)
...
def _series_length(client: ApiClient, date: str, freq: float) -> int:
    result = client.run_operation("legacy_opacity", {"date": date, "hour": 0, "freq": freq})
...
def test_opacity_configured_valid_cases_match_legacy(equivalence_config: dict[str, Any]) -> None:
...
def test_opacity_all_valid_epochs_match_legacy_for_configured_cases(equivalence_config: dict[str, Any]) -> None:
    client = _client(equivalence_config)
    abs_tol = float(equivalence_config.get("abs_tol", 1e-6))
    rel_tol = float(equivalence_config.get("rel_tol", 1e-6))
    ...
    for case in _configured_sweeps(equivalence_config):
        ...
        count = _series_length(client, date, freq)
        assert count > 0
        for hour in range(0, count + 1):
            legacy, native = _run_pair(client, {"date": date, "hour": hour, "freq": freq})
            _assert_finished_results_equivalent(legacy, native, abs_tol=abs_tol, rel_tol=rel_tol)


def test_opacity_expected_async_failures_match_legacy(equivalence_config: dict[str, Any]) -> None:
    client = _client(equivalence_config)
...
@pytest.mark.parametrize("operation", ["legacy_opacity", "opacity"])
def test_opacity_request_validation_cases(equivalence_config: dict[str, Any], operation: str) -> None:
    client = _client(equivalence_config)
