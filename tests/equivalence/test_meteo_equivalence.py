"""Equivalence tests for legacy_meteo and the native Python meteo operation."""
...
def _meteo_config(config: dict[str, Any]) -> dict[str, Any]:
    return dict(config.get("meteo", {}))


def _valid_cases(config: dict[str, Any]) -> list[dict[str, Any]]:
    return list(_meteo_config(config).get("valid_cases", []))


def _configured_dates(config: dict[str, Any]) -> list[str]:
    return list(_meteo_config(config).get("all_valid_epochs_for_dates", []))


def _async_failure_cases(config: dict[str, Any]) -> list[dict[str, Any]]:
    return list(_meteo_config(config).get("async_failure_cases", []))


def _request_validation_cases(config: dict[str, Any]) -> list[dict[str, Any]]:
    return list(_meteo_config(config).get("request_validation_cases", []))


def _run_pair(client: ApiClient, parameters: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    legacy = client.run_operation("legacy_meteo", parameters)
    native = client.run_operation("meteo", parameters)
...
def _series_length(client: ApiClient, date: str) -> int:
    result = client.run_operation("legacy_meteo", {"date": date, "hour": 0})
...
def test_meteo_configured_valid_cases_match_legacy(equivalence_config: dict[str, Any]) -> None:
...
def test_meteo_all_valid_epochs_match_legacy_for_configured_dates(equivalence_config: dict[str, Any]) -> None:
    client = _client(equivalence_config)
    abs_tol = float(equivalence_config.get("abs_tol", 1e-6))
    rel_tol = float(equivalence_config.get("rel_tol", 1e-6))

    for date in _configured_dates(equivalence_config):
        count = _series_length(client, date)
        assert count > 0
        for hour in range(0, count + 1):
            legacy, native = _run_pair(client, {"date": date, "hour": hour})
            _assert_finished_results_equivalent(legacy, native, abs_tol=abs_tol, rel_tol=rel_tol)


def test_meteo_expected_async_failures_match_legacy(equivalence_config: dict[str, Any]) -> None:
    client = _client(equivalence_config)
...
@pytest.mark.parametrize("operation", ["legacy_meteo", "meteo"])
def test_meteo_request_validation_cases(equivalence_config: dict[str, Any], operation: str) -> None:
    client = _client(equivalence_config)
