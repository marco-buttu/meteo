"""Native Python implementation of the legacy meteo operation."""
...
def _build_single_result(*, mdata: MData, epoch_index: int) -> dict[str, float]:
    return {
        "temperature_k": _round_legacy_float(mdata.tmp[0][epoch_index]),
        "dew_point_k": _round_legacy_float(mdata.dpt[0][epoch_index]),
        "relative_humidity_pct": _round_legacy_float(mdata.rh[0][epoch_index]),
        "pressure_hpa": _round_legacy_float(mdata.prs[0][epoch_index]),
        "u_wind_mps": _round_legacy_float(mdata.uwind[0][epoch_index]),
        "v_wind_mps": _round_legacy_float(mdata.vwind[0][epoch_index]),
    }
